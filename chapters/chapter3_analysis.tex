...
In the top/anti-top production (\gls{TTBAR}), the top pair decays into leptons, b-quarks and neutrinos. Figure \ref{fig:fig_3_3} shows the Feynman diagram of the leptonic decay chain. Like in the $Z\to\tau\tau$ process the leptons originate from secondary vertices and have different kinematic properties in comparison to the signal. \\
...
Besides the decay of fermions like the $\tau$ leptons and the tops, also the vector boson can decay leptonically. The possible vector bosons pairs are $ZZ$/$WW$/$WZ$. Figure \ref{fig:fig_3_4} shows the Feynman diagram of vector boson production. For the $WW$ pair, the $W$ bosons decay into two leptons with two neutrinos, for the $ZZ$ pair the Z bosons decay into four leptons, where two leptons are either not identified correctly or are outside the acceptance of the detector, and for the $WZ$ pair the bosons decay into three leptons and a neutrino, one of the leptons is not identified. \\
...
There are in addition processes, in which the end state particles are not identified correctly, like discussed for the $Z\to\ell\ell$ process. One of the processes is the single $W$ boson production in association with a quark, as seen in figure \ref{fig:fig_3_5}. The $W$ boson decays leptonically, but the quark jet is misidentified as a lepton or a \gls{TAUH}. The other process are \gls{QCD} multijet final states, which are the result of the hard interaction of the partons from the protons. These processes lead to quark and gluon jets misidentified as leptons. Figure \ref{fig:fig_3_6} shows two possible Feynman diagrams contributing to the \gls{QCD} background. \\
...
As for other objects the \gls{CMS} collaboration provides \gls{MVA} classifier for \gls{TAUH} \cite{TAURECO}. But due to reconstruction of the \gls{TAUH} from decay products, it happens that other objects like electrons, muons and jets are misidentified as \gls{TAUH}.  \\
