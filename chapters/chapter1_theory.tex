...
\subsection{U(1)${_e}$ symmetry and quantum electrodynamics (\acs{QED})}
...
The discussion in section \ref{sec:section_1_3_2} showed, that lepton flavour symmetry is not absolute and can be broken. In the case of neutrino mixing induced \acs{LFV} non observable rates are predicted. But if \acs{LFV} would be found on a measurable level, it would imply that another process in a beyond the Standard Model (\acs{BSM}) theory exists, which breaks lepton flavour symmetry. So \acs{LFV} can be used as a probe and a getway to \acs{BSM} theories. \\

One example of such a theory is the specific supersymmetric extension of the standard model \cite{SUSYLFV}, which could predict \acs{BSM} \acs{LFV} decays. But a more convenient way to probe \acs{BSM} is to search in a model-independent approach, because a specific \acs{LFV} model is not nessecarily realized, but \acs{LFV} could still exist because of another realisation in nature. So to search for example a \acs{LFV} Z decay without any underlying model give rise to the possibility to find \acs{BSM} physics, without constraints due to the specific model dependent phase space. 
